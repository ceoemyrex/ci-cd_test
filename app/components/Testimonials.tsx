      ...
      <div className="max-w-310 2xl:max-w-350 px-4 mx-auto">
        <header className="text-center max-w-153.75 mx-auto">
          <span className="bg-[#CACACA1A]/10 px-4 text-xs lg:text-sm py-1.5 lg:py-2.5 rounded-[100px] text-secondary inline-flex items-center gap-x-1.5 border border-[#B6DDA8]">
            <HexagonIcon />
            {AppTranslator.getLocaleText({ locale, translations: testimonialsText.badge })}
          </span>
          <div className="mt-6 space-y-4">
            <p className="font-bold text-2xl lg:text-5xl">
              {AppTranslator.getLocaleText({ locale, translations: testimonialsText.title })}
            </p>
            <p className="text-grey text-center text-sm lg:text-lg">
              {AppTranslator.getLocaleText({ locale, translations: testimonialsText.description })}
            </p>
          </div>
        </header>
          ...
          <div className="flex-1 hidden lg:flex flex-col justify-between">
            ...
            <div className="relative h-45.75 bg-theme/10 rounded-2xl lg:rounded-4xl">
              <img
                ...
                alt={AppTranslator.getLocaleText({ locale, translations: testimonialsText.testimonial1.alt })}
              />
            </div>
          </div>
              ...
              <div className="flex-1">
                <div className="bg-theme/10 h-50 lg:h-100 relative rounded-4xl">
                  <img
                    ...
                    alt={AppTranslator.getLocaleText({ locale, translations: testimonialsText.testimonial2.name })}
                  />
                </div>
              </div>
              <div className="flex-1 space-y-4 flex flex-col justify-between">
                <p className="text-sm lg:text-2xl text-[#2D2F36]">
                  {AppTranslator.getLocaleText({ locale, translations: testimonialsText.testimonial2.text })}
                </p>
                <div>
                  <p className="text-base lg:text-2xl text-[#2D2F36] font-medium">
                    {AppTranslator.getLocaleText({ locale, translations: testimonialsText.testimonial2.name })}
                  </p>
                  <div className="lg:flex items-center gap-x-3 text-lg">
                    <p className="text-grey text-sm lg:text-base">
                      {AppTranslator.getLocaleText({ locale, translations: testimonialsText.testimonial2.studio })}
                    </p>
                    <p className="text-[#2D2F36] text-sm lg:text-base">
                      {AppTranslator.getLocaleText({ locale, translations: testimonialsText.testimonial2.location })}
                    </p>
          ...
          <div className="flex-1 hidden lg:flex flex-col-reverse justify-between">
            ...
            <div className="relative h-45.75 bg-theme/10 rounded-4xl">
              <img
                ...
                className="absolute top-0 left-0 w-full h-full object-cover rounded-4xl"
                alt={AppTranslator.getLocaleText({ locale, translations: testimonialsText.testimonial3.alt })}
              />
            </div>
          </div>
        </div>
        ...
        <div className="max-w-129 flex-wrap mx-auto flex justify-center items-center gap-4">
          ...
          <button className="bg-white lg:flex-1 text-dark border border-black/10 text-xs lg:text-lg rounded-xl lg:rounded-2xl px-5 w-full lg:w-auto lg:px-10 py-2.5 lg:py-5 font-medium">
            {AppTranslator.getLocaleText({ locale, translations: testimonialsText.ctaReview })}
          </button>
        </div>
      </div>
    </section>
